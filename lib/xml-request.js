...
exports.xmlRequest = function(options, callback) {
  ...
  if (! options.serviceName) defer.reject({error: new Error("Missing serviceName")});
  if (! options.opType) defer.reject({error: new Error("Missing opType")});
  ...
  request.post(options.reqOptions, function(error, response) {
    ...
    if (response.statusCode !== 200) {
      resObj = new EbayClientError(util.format("Bad response status code", response.statusCode, (response.body ? response.body.toString() : null)));
      defer.reject(resObj);
    }

    // raw XML wanted?
    if (options.raw) {
      resObj = {error: null, data: response.body};
