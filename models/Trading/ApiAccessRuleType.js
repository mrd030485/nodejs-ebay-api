function ApiAccessRuleType(CallName, CountsTowardAggregate, DailyHardLimit, DailySoftLimit, DailyUsage, HourlyHardLimit, HourlySoftLimit, HourlyUsage, Period, PeriodicHardLimit, PeriodicSoftLimit, PeriodicUsage, PeriodicStartDate, ModTime, RuleCurrentStatus, RuleStatus) {
	...
	/**
	  Documentation
	   This type is used by the <b>ApiAccessRule</b> container that is returned in the <b>GetApiAccessRules</b> call response. An <b>ApiAccessRule</b> container will be returned for every Trading API call that
                    the user/application has used. The <b>ApiAccessRule</b> container consists of hourly, daily, and periodic call limits for the call, the total number of times the call was used, and other information about usage of the call.
	 */
...
ApiAccessRuleType.prototype.toJSON = function(with_null) {
	var json = { 
		...
		RuleCurrentStatus: (this.RuleCurrentStatus === undefined)? null : this.RuleCurrentStatus,
		RuleStatus: (this.RuleStatus === undefined)? null : this.RuleStatus
	};
	if(!with_null) {
		for(var k in json) {
			if(json[k] === null) {
